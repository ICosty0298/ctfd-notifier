...
def load_bp(plugin_route):
    @notifier_bp.route(plugin_route, methods=["GET"], endpoint="notifier_get_notify_config")
    @admins_only
    def get_notify_config():
        config = DBUtils.get_notify_config()
        return render_template("ctfd_notifier/config.html", config=config)

    @notifier_bp.route(plugin_route, methods=["POST"], endpoint="notifier_update_config")
    @admins_only
    def update_config():
